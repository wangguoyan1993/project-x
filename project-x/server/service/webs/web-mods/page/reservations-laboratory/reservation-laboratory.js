...
let reservationMods = function () {
    ...
    let _reservationEvent = () => {
        ...
        let index = ibcpLayer.ShowDiv('reservations-page.html','预约实验室', '400px', '350px',function(){
            //加载所选数据信息
            $('#code').val(selectData.code);
            $('#name').val(selectData.name);
            $('#college').val(selectData.college);
            $('#place').val(selectData.place);

            //确认按钮
            $('#confirm').on('click',function(){
                ibcpLayer.Close(index);
                ibcpLayer.ShowMsg('预约成功！');
                //更新信息（预约成功实验室可在个人信息中看到）
            });
